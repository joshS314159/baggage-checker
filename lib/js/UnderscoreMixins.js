(function UnderscoreMixins(logger, modules){
    ...
    const _ZipObject_ = (function ZipObject(){
        ...
        const setKeyToValues = function createKeyValue(key, values){
        ...
        const zipObject = function zipObject(){
            ...
            // return _.zip(args);
            const objectified = _.map(args, a => {
                return setKeyToValues(_.first(a), _.last(a));
            });

            const zipped = _.zip(...objectified);
            
            return _.map(zipped, array => Object.assign(_.first(array), _.last(array)))
        }
