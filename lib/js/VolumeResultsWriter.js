MODULES.VolumeResultsWriter = (function VolumeResultsWriter($$, logger, modules){
    ...
    const clearEntries = entries => _.map(entries, clearEntry);
    ...
    const appendVolumeRowToColumn = volume => $$.luggage.volume.airlineVolume.append(volume);
    
    const appendVolume = _.compose(appendVolumeRowToColumn, formatVolume);
    
    const appendAirlineNameRowToColumn = airlineName => $$.luggage.volume.airlineName.append(airlineName);
    
    const appendAirlineName = _.compose(appendAirlineNameRowToColumn, formatAirlineName);
    
    const getUnmarkedKeysAppendFn = inequality => _.omit(MARKED_FUNCTIONS, inequality);
    
    const getMarkedKeyAppendFn = inequality => MARKED_FUNCTIONS[inequality];
    
    const setUnmarkedColumns = unmarkedKeysFn => _.map(unmarkedKeysFn, (appendFn,key) => appendFn( unmarkedSymbol() ) );
    
    const setMarkedColumn = markedKeyFn => markedKeyFn(markedSymbol());
    ...
    const setUnmarkedCategory = _.compose(setUnmarkedColumns, getUnmarkedKeysAppendFn, parseInequalityFromInput);
    
    const setMarkedCategory = _.compose(setMarkedColumn, getMarkedKeyAppendFn, parseInequalityFromInput);
    ...
    const setInputVolumeHtml = function setInputVolumeHtml(volume){
        return $$.userInputVolume.html(volume);
    }
    
    const calculatedVolumeFromInput = _.compose(setInputVolumeHtml, wrapInSpan, toFixed1);
